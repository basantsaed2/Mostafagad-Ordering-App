 ...
 useEffect(() => {
  ...
  // Delete location and then refetch locations
    const handleDelete = async (id, name) => {
       const success = await deleteData(`https://mostafagadbcknd.food2go.online/customer/address/delete/${id}`, `${name} Deleted Success.`);
       if (success) {
  ...
  return (
    ...
    <div className='w-full flex flex-col gap-3 mb-5'>
        <div className="w-full flex flex-col lg:flex-row gap-5 p-4">
            ...
            {/* Navbar Type Order */}
            <div className="w-full lg:w-1/2 flex flex-col gap-5 items-center justify-center gap-x-3 pt-4 xl:p-6">
                <h1 className='text-2xl font-semibold'>PickUP Or Delivery</h1>

                <div className="w-full flex flex-wrap gap-4 justify-center items-center gap-x-4 md:gap-x-6">
                    {orderTypes.map((type) => (
                    type.status === 1 && (
                        <span
                        key={type.id}
                        ...
                        {type.type.charAt(0).toUpperCase() + type.type.slice(1) === "Dine_in" ? <MdOutlineTableRestaurant size={64} /> : null}
